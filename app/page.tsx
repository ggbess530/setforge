...
const FEATURES = [
  { icon:'🎵', title:'Picks the perfect tracks for you', plain:'No more hours of searching for songs that go together.', desc:"Our AI listens to your vibe — genre, mood, crowd — and selects real tracks that actually flow together. Like having a DJ friend who knows everything." },
  { icon:'🔗', title:'Songs that sound amazing back-to-back', plain:'Ever noticed how some songs clash when mixed? This stops that.', desc:"SetForge automatically orders tracks so the musical keys are compatible. Every transition sounds intentional — even if you've never mixed before." },
  { icon:'🔄', title:"Don't like a song? Swap it instantly", plain:"You're always in control.", desc:"If one track doesn't feel right, hit swap. The AI replaces it with something that fits the same energy and sound — without breaking the flow." },
  { icon:'📈', title:'Sets that build to a peak', plain:'Great DJ sets tell a story. Yours will too.', desc:'Choose how your set should feel — a slow build to peak energy, a consistent vibe, or a cool-down. SetForge shapes every track selection around that journey.' },
  { icon:'☁️', title:'Your sets saved forever', plain:'Come back any time and pick up where you left off.', desc:"Every set you create is saved to your personal library in the cloud. Rename them, load them, share them — they're yours forever." },
  { icon:'📤', title:'Ready to take to your DJ software', plain:'Works with Rekordbox, Serato, Traktor, and more.', desc:'Export any set as a formatted list you can take straight into your DJ software — or just copy and paste it anywhere you need it.' },
]

const FAQS = [
  { q:'Do I need to be a DJ to use SetForge?', a:"Not at all. SetForge was built with beginners in mind. If you love music and want to put together a great playlist or DJ set — even your first one ever — SetForge will do the hard work for you. No experience needed." },
  { q:'What is BPM and why does it matter?', a:"BPM stands for Beats Per Minute — it's basically how fast a song is. Most club and dance music sits between 120–130 BPM. When songs in a DJ set have similar BPMs, the mix sounds smooth instead of jarring. SetForge handles all of this automatically." },
  { q:'What is harmonic mixing? Do I need to understand it?', a:"Harmonic mixing means choosing songs that are in compatible musical keys — so they sound good played back-to-back. Professional DJs spend years learning this. SetForge does it instantly for every track in your set, so you get pro-quality transitions without knowing any music theory." },
  { q:'Do I need any DJ equipment or software?', a:"No equipment needed to use SetForge — it works in any web browser on any device. If you do have DJ software like Rekordbox or Serato, you can export your set directly to it. But you can also just use SetForge to plan your set and find great music." },
  { q:'Will the AI suggest real songs I can actually find?', a:"Yes. SetForge suggests real, well-known tracks that exist on Beatport, Spotify, and major platforms. Every track card has direct links so you can preview and buy the songs instantly. Think of it as AI-powered music discovery." },
  { q:'What is an "energy arc" and which should I pick?', a:'"Energy arc" is just how your set builds and flows over time. "Slow Build" means starting mellow and getting more intense — great for warming up a crowd. "Peak Time" keeps energy high throughout. "Cool Down" works for the end of a night. "Wave" goes up and down. When in doubt, pick Slow Build — it\'s the most crowd-pleasing.' },
  { q:'How is SetForge different from just making a Spotify playlist?', a:"A Spotify playlist is just songs in a row. SetForge creates a DJ set — every track is chosen for its BPM, musical key, energy level, and how it flows into the next one. You also get detailed transition notes telling you exactly how to mix between tracks. It's the difference between a list and a performance." },
  { q:'Is there a free plan?', a:"Yes — SetForge has a free plan that gives you 5 set generations every month, forever. No credit card, no trial expiry. You also get a 7-day Pro trial when you first sign up so you can try unlimited generations before deciding." },
]
...
const STATS = [
  ...
  { n:'Free', label:'forever plan available' },
]

const TIERS = [
  {
    name:'Free', price:'$0', period:'forever', highlight:false, badge:null, color:'#9a9ab8',
    desc:'Perfect for getting started.',
    perks:["5 set generations per month","Swap tracks you don't like",'Save sets to your library','Export to DJ software','Public set sharing'],
    cta:'Start free — no card needed',
  },
  {
    name:'Pro', price:'$9', period:'/month', highlight:true, badge:'Most popular', color:M,
    desc:'For DJs who create sets regularly.',
    perks:['Unlimited set generations','Unlimited track swaps','Everything in Free','Priority generation speed','Early access to new features'],
    cta:'Start 7-day free trial',
  },
  {
    name:'Team', price:'$19', period:'/month', highlight:false, badge:null, color:C,
    desc:'For agencies and DJ teams.',
    perks:['Everything in Pro','Team member access','Custom branding on exports','Dedicated support'],
    cta:'Start 7-day free trial',
  },
]
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#06060c', color:'#e8e8f0', fontFamily:"'Inter',system-ui,sans-serif", overflowX:'hidden' }}>
      <style>{`
        ...
        .tier-card { transition:transform .2s; }
      ...
      <nav style={{ position:'sticky', top:0, zIndex:50, borderBottom:'1px solid #1a1a2e', backdropFilter:'blur(16px)', background:'rgba(6,6,12,.85)', padding:'0 24px' }}>
        <div style={{ maxWidth:1100, margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', height:64 }}>
          ...
          <div style={{ display:'flex', gap:10, alignItems:'center' }}>
            {isSignedIn ? (
              <>
                <Link href="/app"><button className="btn-primary" style={{ padding:'9px 22px', borderRadius:8, fontSize:14, fontWeight:600 }}>Open App →</button></Link>
                <UserButton />
              </>
            ) : (
              <>
                ...
                <SignUpButton mode="modal"><button className="btn-primary" style={{ padding:'9px 22px', borderRadius:8, fontSize:14, fontWeight:600 }}>Start free →</button></SignUpButton>
              </>
            )}
          </div>
        </div>
      ...
      <div style={{ position:'relative', zIndex:1 }}>
        ...
        <section style={{ textAlign:'center', padding:'90px 24px 70px', maxWidth:780, margin:'0 auto' }}>
          ...
          <h1 style={{ fontSize:'clamp(38px,6vw,72px)', fontWeight:700, lineHeight:1.15, margin:'0 0 24px', letterSpacing:'-0.02em' }}>
            ...
            <span style={{ background:`linear-gradient(90deg,${M},${C})`, WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>in about 60 seconds.</span>
          </h1>
          <p style={{ fontSize:19, color:'#9a9ab8', maxWidth:560, margin:'0 auto 16px', lineHeight:1.7 }}>
          ...
          <div style={{ display:'flex', gap:14, justifyContent:'center', flexWrap:'wrap' }}>
            {isSignedIn ? (
              <Link href="/app"><button className="btn-primary" style={{ padding:'16px 36px', borderRadius:10, fontSize:16, fontWeight:700 }}>Open SetForge →</button></Link>
            ) : (
              <SignUpButton mode="modal"><button className="btn-primary" style={{ padding:'16px 36px', borderRadius:10, fontSize:16, fontWeight:700 }}>Start free — no card needed →</button></SignUpButton>
            )}
            <a href="#how-it-works" style={{ textDecoration:'none' }}>
              <button className="btn-ghost" style={{ padding:'16px 32px', borderRadius:10, fontSize:15 }}>See how it works</button>
            </a>
          </div>
          <p style={{ marginTop:24, fontSize:13, color:'#4a4a66' }}>Free plan available · 7-day Pro trial · No credit card</p>
        </section>
        ...
        <section style={{ padding:'20px 24px 60px', maxWidth:900, margin:'0 auto' }}>
          <div style={{ background:'linear-gradient(135deg,#0d0d1a,#0a0a14)', border:'1px solid #1f1f38', borderRadius:20, padding:'48px 40px', position:'relative', overflow:'hidden' }}>
            ...
            <div style={{ position:'relative' }}>
              ...
              <h2 style={{ fontSize:'clamp(26px,4vw,40px)', fontWeight:700, margin:'0 0 16px', lineHeight:1.2 }}>You don't need to know anything about DJing to start.</h2>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:14 }}>
                {['No library to import','No music theory knowledge','No DJ software required','No experience necessary'].map(text => (
                  <div key={text} style={{ display:'flex', alignItems:'center', gap:10, fontSize:15, color:'#c8c8e0' }}>
                    <span>✅</span>{text}
                  </div>
                ))}
              </div>
        ...
        <section id="how-it-works" style={{ padding:'40px 24px 60px', maxWidth:1000, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))', gap:20 }}>
            {[
              { n:'1', color:M, title:'Tell it your vibe', desc:'Pick a genre (or describe your own), choose your crowd type, set the mood. Even something like "dark and hypnotic warehouse techno" works perfectly.', tag:'Takes 30 seconds' },
              { n:'2', color:C, title:'AI builds your set', desc:'SetForge picks real tracks, orders them so they flow together musically, shapes the energy across your set, and writes transition notes for each song.', tag:'Ready in ~20 seconds' },
              { n:'3', color:M, title:'Tweak and take it', desc:"Swap tracks you don't love, drag them into your preferred order, lock your favourites. Then copy, export, or share your finished set.", tag:'Your set, your way' },
            ].map(step => (
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:2 }}>
            {FAQS.map((faq, i) => (
              <div key={i} className="faq-row" onClick={() => setOpenFaq(openFaq === i ? null : i)} style={{ background:'#0a0a14', borderRadius:i===0?'14px 14px 0 0':i===FAQS.length-1?'0 0 14px 14px':'0', border:'1px solid #1a1a2e', borderTop:i===0?undefined:'none', overflow:'hidden' }}>
                ...
                {openFaq === i && <div style={{ padding:'0 24px 20px', fontSize:15, color:'#9a9ab8', lineHeight:1.75 }}>{faq.a}</div>}
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="pricing" style={{ padding:'40px 24px 60px', maxWidth:1000, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:20 }}>
            {TIERS.map(tier => (
              <div key={tier.name} className="tier-card" style={{ background:'#0a0a14', border:`1.5px solid ${tier.highlight ? M : '#1a1a2e'}`, borderRadius:18, padding:32, position:'relative', boxShadow:tier.highlight?`0 0 50px ${M}18`:'none' }}>
                {tier.badge && (
                ...
                <SignUpButton mode="modal">
                  <button className={tier.highlight?'btn-primary':'btn-ghost'} style={{ width:'100%', padding:'14px 0', borderRadius:10, fontSize:15, fontWeight:600 }}>{tier.cta}</button>
                </SignUpButton>
              </div>
            ))}
          </div>
          <p style={{ textAlign:'center', fontSize:13, color:'#4a4a66', marginTop:24 }}>Pro and Team plans include a 7-day free trial. Free plan never expires.</p>
        </section>
        ...
        <section style={{ padding:'60px 24px 100px', textAlign:'center' }}>
          <div style={{ maxWidth:600, margin:'0 auto', background:'linear-gradient(135deg,#0d0d1a,#0a0a14)', border:'1px solid #1f1f38', borderRadius:24, padding:'56px 40px', position:'relative', overflow:'hidden' }}>
            ...
            <div style={{ position:'relative' }}>
              <div style={{ fontSize:13, color:C, fontWeight:600, letterSpacing:2, marginBottom:16, textTransform:'uppercase' }}>✦ Free forever plan</div>
              <h2 style={{ fontSize:'clamp(28px,5vw,44px)', fontWeight:700, margin:'0 0 16px', lineHeight:1.2 }}>Ready to build your first set?</h2>
              <p style={{ fontSize:16, color:'#6a6a8a', marginBottom:32, lineHeight:1.6 }}>
                Join DJs and music lovers already using SetForge. Start free — no credit card, no time limit.
              </p>
              <SignUpButton mode="modal">
