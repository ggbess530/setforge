...
// (ReccoBeats' audio-features response reuses this exact integer scheme.)
export const SPOTIFY_TO_CAMELOT: Record<string, string> = {
...
export function normalizeArtists(s: string): string[] {
  return (s || '')
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
...
export type SpotifyTrack = { id: string; name: string; artists: { name: string }[]; album: { name: string }; external_urls: { spotify: string } }
...
// Tiered, early-exit cascade: strict field search -> normalized free text -> title-only.
// Pools candidates across every tier it runs and returns the single best-scoring match.
export async function findSpotifyTrack(artist: string, title: string, token: string): Promise<{ track: SpotifyTrack; score: number } | null> {
  ...
  if (!best || best.score < MIN_ACCEPTABLE) return null
  return best
}
